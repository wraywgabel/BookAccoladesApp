...
if choice == "Top by Accolades":
    ...
    st.dataframe(
    display_table_unique[["Title", "Author", "Year", "Category", "Result"]],
...
if choice == "Your Books" and 'line' in locals():
    ...
    if st.checkbox("Show all books with accolades"):
        ...
        with st.container():
            st.dataframe(all_agg[["Title", "Author", "Accolade Count"]], use_container_width=True)
...
if choice in ["Top by Accolades", "Search Books"] and base_df is not None:
    ...
    if selected_books.empty:
        ...
        book_table = accolades_rows[["Accolade", "Category", "Year", "Result"]].copy()

        # Format Year without commas
        if "Year" in book_table.columns:
            book_table["Year"] = book_table["Year"].apply(lambda x: str(int(x)) if pd.notna(x) else "")

        st.dataframe(
            book_table.sort_values("Year", ascending=False),
            use_container_width=True,
        )
